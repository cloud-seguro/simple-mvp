...
export function CybersecurityResults({ quizData, results, userInfo, onRestart }: CybersecurityResultsProps) {
      ...
      <main className="flex-grow p-8">
        <div className="max-w-2xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
            <div>
              <h1 className="text-3xl font-bold mb-2">{userInfo.firstName}, aquí están sus resultados</h1>
              <p className="text-lg text-gray-600">Evaluación de Madurez en Ciberseguridad</p>
            </div>
            ...
            <div className="bg-secondary p-6 rounded-md">
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <div>
                  ...
                  <p className={`text-3xl font-bold ${maturity.color}`}>{maturity.level}</p>
                </div>
              </div>
              <div className="w-full bg-muted rounded-full h-4">
                <div
                  className="bg-primary h-4 rounded-full transition-all duration-1000"
                  style={{ width: `${(overallScore / maxPossibleScore) * 100}%` }}
                />
              </div>
            </div>

            <div className="bg-secondary p-6 rounded-md">
              <h2 className="text-xl font-semibold mb-4">Desglose por Categoría</h2>
